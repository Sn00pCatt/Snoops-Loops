...
import loops.sn00p.api.forge.concurrency.UtilForgeConcurrency;
...
public class ForgeGui implements Gui {
    ...
    private final PlayerManager<ForgeSnoopPlayer, EntityPlayerMP> playerManager;
    private final Consumer<ForgeSnoopPlayer> closeConsumer;
    ...
    ForgeGui(String title, int height, PlayerManager<ForgeSnoopPlayer, EntityPlayerMP> playerManager,
             Consumer<ForgeSnoopPlayer> closeConsumer, Pane... panes) {
    ...
    @Override
    public void open(SnoopPlayer<?> player) {
        if (!(player instanceof ForgeSnoopPlayer)) {
            return;
        }

        UtilForgeConcurrency.runLater(() -> {
            EntityPlayerMP parent = ((ForgeSnoopPlayer) player).getParent();
    ...
    private final class ForgeGuiContainer extends Container {
        ...
        public ItemStack slotClick(int slot, int dragType, ClickType clickTypeIn, EntityPlayer player) {
            ...
            SnoopPlayer<?> snoopPlayer = this.gui.playerManager.getPlayer((EntityPlayerMP) player);
            ...
            for (ForgeSimplePane pane : this.gui.panes) {
                ...
                simpleDisplayableSlot.setClicks(simpleDisplayableSlot.getClicks() - 1);
                simpleDisplayableSlot.getDisplayable().onClick(snoopPlayer, clickType);
                ForgeGuiTracker.enqueueUpdate(snoopPlayer);
            }
        ...
        public void onContainerClosed(EntityPlayer playerIn) {
            ...
            ForgeSnoopPlayer player = this.gui.playerManager.getPlayer(playerIn.getUniqueID());
