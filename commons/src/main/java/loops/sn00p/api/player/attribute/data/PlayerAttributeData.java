package loops.sn00p.api.player.attribute.data;
...
public class PlayerAttributeData {
    ...
    private Constructor<? extends PlayerAttribute<?>> getConstructor() {
        try {
            return attributeClass.getConstructor(this.managerClass, SnoopPlayer.class);
    ...
    /**
     *
     * Creates a new instanceof the {@link PlayerAttribute} for the {@link SnoopPlayer} passed
     * {@link PlayerAttribute#load} not called here
     *
     * @param player The parent of the new attribute
     * @return The new attribute
     */
    public PlayerAttribute<?> getInstance(SnoopPlayer<?> player) {
        try {
